...
Similar to natural language processing, 🤗 Transformers can be applied to tackle some of the most common audio tasks that you're likely to encounter in the wild. Here are some examples:
...
In this section we'll show you how to load audio datasets from the Hugging Face Hub, and how to analyze them with the `pipeline()` function from 🤗 Transformers. Let's start by loading some audio data and exploring its features.
...
✏️ **Try it out!** Download another dialect or language of the MINDS-14 dataset and listen to some samples to get a sense of the variation in the whole dataset. You can find the full list of available languages [here](https://huggingface.co/datasets/PolyAI/minds14).
