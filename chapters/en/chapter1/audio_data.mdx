...
By nature, a sound wave is a continuous signal, meaning it contains an infinite number of signal values in a given time. This poses problems for computer devices which expect finite arrays. To work with it digitally, it needs to be converted into as a series of
...
scale the amplitude absolute value scaled relative to `ref`.
The decibel scale compresses the dynamic range of the amplitude spectrogram, making it easier to see the finer
details in the spectrogram.
