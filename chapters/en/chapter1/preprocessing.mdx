...
As you recall, these spectrograms are a visual representation of the frequencies of a signal. Ultimately, the log-mel
spectrogram is the form of input expected by the Whisper model.

<!--  TODO: not sure if the values are normalized or not -->
Finally, zero-mean unit-variance normalization is applied to the values.
...
As you can see, we now have log-mel spectrogram as `input_features` in the dataset. Let's visualize it for one of the examples:
...
>>> librosa.display.specshow(np.asarray(input_features[0]), x_axis='time', y_axis='mel', sr=16000)
>>> plt.colorbar()
```
