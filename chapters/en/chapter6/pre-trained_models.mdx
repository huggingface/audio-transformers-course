...
[X-Vectors](https://www.danielpovey.com/files/2018_icassp_xvectors.pdf) are a SOTA method that shows superior performance 
...
MMS model has been added to 🤗 Transformers very recently, so the API may still slightly change. At the moment of this Unit's 
...
Wunderbar! If you'd like to try MMS with another language, find other suitable `vits` checkpoints checkpoints [on 🤗 Hub](https://huggingface.co/models?filter=vits).
