...
Now that you know what Text-to-speech (TTS) task is, let's see how we can fine-tune a TTS model. 

In this example, we'll take SpeechT5 that is pre-trained on a combination of speech-to-text and text-to-speech data, 
allowing it to learn a unified space of hidden representations shared by both text 
...
[//]: # (Picking a pretrained model for tts)
