...
For optimal results, you should train an X-vector model on the target speech first. This will ensure that the model 
is better able to capture the unique voice characteristics present in the Dutch language.

[//]: # (TODO: We should probably link to some resource that illustrated how to train an X-vector model to get speaker embeddings)
