...
![Whisper is a transformer encoder-decoder model](https://huggingface.co/blog/assets/111_fine_tune_whisper/whisper_architecture.svg)
...
![The audio waveform gets mapped to a shorter sequence of hidden-states](../../../assets/img/speecht5_decoding.png)
...
During training of the TTS model, the targets are also spectrograms. However, at inference time we want to convert the output spectrogram into an audio waveform, so that we can actually listen to it. For this an external model is used, the **vocoder**. This vocoder is not part of the seq2seq architecture and is trained separately.
