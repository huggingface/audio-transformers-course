...
In this section, we'll cover how the `pipeline()` to leverage pre-trained models for speech recognition. In Unit 2, we
introduced the `pipeline()` as an easy way of running speech tasks, with all pre- and post-processing handled under-the-hood,
as well as giving you the flexibility to quickly experiment with pre-trained checkpoints on the Hugging Face Hub.
...
Let's load a small excerpt of the [LibriSpeech](https://huggingface.co/datasets/librispeech_pipe) dataset to demonstrate
Wav2Vec2's speech transcription capabilities:

```python
from datasets import load_dataset

dataset = load_dataset("hf-internal-testing/librispeech_pipe_demo", "clean", split="validation")
dataset
```
...
The remainder of this section will show you how to use the pre-trained Whisper models for speech recognition using 🤗 Transformers.
In many situations, the pre-trained Whisper checkpoints are extremely performant and give great results, thus we encourage you
to try using the pre-trained checkpoints as a first step to solving any speech recognition problem. Through fine-tuning, the
pre-trained checkpoints can be adapted for specific datasets and languages to further improve upon these results.
...
Hold on a second... we put a Spanish audio input in, but got an English transcription out? 🤔 Whisper has implicitly performed
...
longer audiobook passages, and the resulting audio should be coherent across the entire length.
...
target_length_in_samples = target_length_in_m * 60 * 16000
long_audio = []
...
1. Whisper is inherently designed to work with 30 second samples: anything shorter than 30s is padded to 30s with silence, anything longer than 30s is truncated to 30s by cutting of the extra audio
2. Memory in a transformer block scales with the sequence length squared: doubling the input length quadruples the memory requirement, so passing super long audio files is bound to lead to an out-of-memory (OOM) error
...
at the boundaries, as we can match the overlapping transcriptions together:
...
providing a large computational speed-up compared to transcribing them sequentially.

To activate long-form transcriptions, we have to add one additional argument when we call the pipeline. This argument,
`chunk_length_s`, controls the length of the chunked segments. For Whisper, 30s chunks are optimal, since this matches
the input length Whisper expects.

To activate batching, we pass the argument `batch_size` to the pipeline. Putting it all together, we can transcribe the
long audio sample as follows:

```python
pipe(long_audio, max_new_tokens=256, generate_kwargs={"task": "transcribe"}, chunk_length_s=30, batch_size=8)
```
**Output:**
```
...
Activating timestamp prediction is straightforward, we just need to set the argument `return_timestamps=True`. Timestamps are compatible
with both the chunking and batching methods we used previously, so we can simply append the timestamp argument to our previous call:

```python
pipe(long_audio, max_new_tokens=256, generate_kwargs={"task": "transcribe"}, chunk_length_s=30, batch_size=8, return_timestamps=True)["chunks"]
```
**Output:**
```
[{'timestamp': (0.0, 26.4),
