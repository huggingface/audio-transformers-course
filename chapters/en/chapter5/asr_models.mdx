...
In this section, we'll cover how the `pipeline()` to leverage pre-trained models for speech recognition. In [Unit 2](../chapter2/asr_pipeline.mdx),
...
force it to perform the task of _speech recognition_, which transcribes the audio in the same language it was spoken.
To achieve this, we'll forward the `task` as a generation argument, which is set to `"transcribe"` (for speech recognition):
...
recognition, we can pick our task depending on our needs. Either we translate from audio in any language X to text in
English, or we recognise from audio in language X to text in the same language X.
...
a viable model for many speech recognition tasks without the need for fine-tuning. The `pipeline()` method provides an easy
way of running inference in one-line API calls with control over the generated predictions.
