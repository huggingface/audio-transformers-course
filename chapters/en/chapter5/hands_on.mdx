...
* Fine-tune the `”openai/whisper-tiny”` model using the American English ("en-US") subset of the `”PolyAI/minds14”` dataset. Use the first **450 examples for training**, and the rest for evaluation.
...
Once you have fine-tuned a model, make sure to upload it to the :hugging_face: Hub with the following `kwargs`:
```
kwargs = {
...
```
You will pass this assignment if your model’s WER is lower than **0.37**.
