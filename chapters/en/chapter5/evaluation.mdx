...
section on fine-tuning by evaluating the pre-trained Whisper model on the Common Voice 17 Dhivehi test set. We'll use
...
Next, we'll load the Dhivehi test split of Common Voice 17. You'll remember from the previous section that the Common
Voice 17 is *gated*, meaning we had to agree to the dataset terms of use before gaining access to the dataset. We can
...
<Tip>
    ...
    on the Hugging Face Hub through the following link: https://huggingface.co/datasets/mozilla-foundation/common_voice_17_0
</Tip>
