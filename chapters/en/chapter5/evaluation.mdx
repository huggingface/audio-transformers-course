...
over the input audios, rather than inferring just the first one. We first define a helper function that yields examples
from our dataset using a generator object, and then pass the generator through the pipeline to get our predictions and
references. The following code cell will take approximately five minutes if running on a GPU with half-precision, peaking
at 12GB memory:
...
def data(dataset):
    """Helper function that wraps a dataset as a generator object for compatibility with pipeline"""
...
for out in tqdm(
    pipe(
        ...
        max_new_tokens=128,
        generate_kwargs={"task": "transcribe", "language": "sinhalese"},
        batch_size=32,
    ),
    total=len(common_voice_test),
):
    all_predictions.append(out["text"])
    all_references.append(out["reference"][0])
```

<Tip>
    If you experience a CUDA out-of-memory (OOM) when running the above cell, incrementally reduce the `batch_size` by
    factors of 2 until you find a batch size that fits in your device.
</Tip>
...
Okay... 168% essentially means our model is outputting garbage 😜 Not to worry, it'll be our aim to improve this by
