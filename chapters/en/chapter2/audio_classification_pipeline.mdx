...
Audio classification task involves assigning one or more labels to an audio recording based on its content. The labels
...
In NLP, pre-trained transformer models, such as the ones based on the BERT or GPT architectures, have proven to be highly
effective in natural language processing tasks such as text classification, question answering, and language translation.
Turns out, transformer architectures can also be applied to audio and multimodal tasks by treating audio as sequences of data and
leveraging the transformer's ability to learn from sequential data.
...
By default, this pipeline uses a model trained for a task known as _keyword spotting_, which involves detecting a specific word in
an audio recording. In our case, we need a model that's been fine-tuned for intent classification, and specifically on
