...
speech from one language into speech in a **different** language:
...
can speak it directly and have a STST system convert your spoken speech into the target language. The recipient can then
...
language Y, but we leave this as an extension to the reader and provide pointers where applicable. We further divide up the 
...
Italian (`it`) split of the [VoxPopuli](https://huggingface.co/datasets/qmeeus/voxpopuli) dataset:
...
To listen to this sample, we can either play it using the dataset viewer on the Hub: [qmeeus/voxpopuli/viewer](https://huggingface.co/datasets/qmeeus/voxpopuli/viewer/it/validation?row=0)
...
<Tip>
    ...
    use an MMS TTS checkpoint pre-trained in your target language.
</Tip>
...
speech and follow the tips provide for translating into a language of your choice!
