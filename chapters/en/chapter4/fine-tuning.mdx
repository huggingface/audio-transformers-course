...
Training will take approximately 30 minutes depending on your GPU or the one allocated to the Google Colab. Our best
evaluation accuracy is 75% - not bad for just 5 epochs with 899 examples of training data! We could certainly improve
upon this result by training for more epochs, using regularisation techniques such as _dropout_, or sub-diving each
audio example from 30s into 15s segments to change our data pre-processing strategy.
