...
The ten keywords are single words that would typically be used in on-device settings to control basic tasks or launch
other processes. A similar model is running continuously on your mobile phone. Here, instead of having command words, we
have 'wake words' specific to your device, such as "Hey Google" or "Hey Siri". When the audio classification model detects
these wake words, it triggers your phone to start listening to the microphone and transcribe your speech using a speech
recognition model. The audio classification model is much smaller and lighter than the speech recognition model, often
only several millions of parameters compared to several hundred millions for speech recognition. Thus, it can be run
continuously on your device without draining your battery! Only when the wake word is detected is the larger speech
recognition model launched, and afterwards it is shut down again.
...
it's the right dataset for your needs. Once you've selected a dataset, it's trivial to load the data so that you can start using it.
...
We'll load the official checkpoint from the [SUPERB benchmark](https://arxiv.org/abs/2105.01051), [`"superb/hubert-base-superb-ks"`](https://huggingface.co/superb/hubert-base-superb-ks),
and run the example through the pipeline:

```python
classifier = pipeline("audio-classification", model="superb/hubert-base-superb-ks")
classifier(sample["file"])
```
**Output:**
```
TODO(SG)
...
an LID model can be used to categorise the language(s) spoken in the audio sample, and then an appropriate speech recognition
model selected to transcribe the audio.
