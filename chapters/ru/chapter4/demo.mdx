...
Первое, что необходимо сделать, это загрузить тонко настроенную контрольную точку с помощью класса `pipeline()` - это уже хорошо знакомо по разделу
[предобученние модели](classification_models). Вы можете изменить `model_id` на пространство имен вашей доработанной модели на Hugging Face Hub:
