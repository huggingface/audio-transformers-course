...
В данном примере мы возьмем подмножество голландского (`nl`) языка из датасета [VoxPopuli](https://huggingface.co/datasets/qmeeus/voxpopuli).
[VoxPopuli](https://huggingface.co/datasets/qmeeus/voxpopuli) - это обширный многоязычный речевой корпус, состоящий из данных,
...
Примеры из датасета содержат признак `text`, который мы будем использовать в качестве входного текста. Важно знать, 
что в токенизаторе SpeechT5 нет токенов для чисел, поэтому если ваш текст содержит числа, возможно, их придется записать в виде текста.
