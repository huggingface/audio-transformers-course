...
CTC (Connectionist Temporal Classification) или Коннекционистская Временная Классификация  это техника, которая используется в моделях трансвормеров состоящих только из енкодера испольуемых в задачах автоматического распознавания речи (ASR). Примерами таких моделей являются **Wav2Vec2**, **HuBERT** и **M-CTC-T**.
