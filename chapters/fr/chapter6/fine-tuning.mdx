...
Pour cet exemple, nous prendrons le sous-ensemble en néerlandais (`nl`) du jeu de données [VoxPopuli](https://huggingface.co/datasets/qmeeus/voxpopuli).
[VoxPopuli](https://huggingface.co/datasets/qmeeus/voxpopuli) est un corpus de parole multilingue à grande échelle composé de données provenant d'enregistrements d'événements du Parlement européen de 2009 à 2020.
...
Les exemples du jeu de données contiennent une caractéristique `text` que nous utiliserons comme entrée de texte. Il est important de savoir que le *tokenizer* de SpeechT5 n'a pas de *tokens* pour les nombres, donc si votre texte contient des nombres, vous devrez peut-être les écrire sous forme textuelle.
